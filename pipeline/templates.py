...
Templates live as Markdown files in pipeline/templates/ (first line `subject: ...`,
...
Usage:
    python pipeline/templates.py load                     load the files into the database
    python pipeline/templates.py list
    python pipeline/templates.py render judge-invite --var first_name=Ada --var search_name="..."
    python pipeline/templates.py lint path/to/draft.txt   lint any text file
...
def main(argv=None) -> int:
    ...
    sub.add_parser("load", help="load pipeline/templates/*.md into the database")
