...
Usage:
    python pipeline/verify.py check --name "Ada Example" --org "Example Institute" [--email a@example.org] [--record]
    python pipeline/verify.py batch --search SEARCH [--record]     verify every prospect in a search
    python pipeline/verify.py log [--candidate ID]                  show the verification log
"""
