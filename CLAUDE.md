...
**Path:** `data/ops.db` at the repo root (SQLite, WAL mode). Resolved by `platform/core/paths.py`: the `OPS_ROOT` env var if set, else walking up from the module to the repo root. Always use `platform/tools/query.py` or `paths.DB_PATH`; never hardcode an absolute path.

**Query tool (always use this):**
```bash
python platform/tools/query.py "SQL HERE"
```
...
query.py episodes "keyword"         # FTS episodes search (topic + summary). For semantic: platform/search/embed_episodes.py --search
query.py health                     # system health snapshot
query.py explain AI-xxx             # action item timeline
```

**Task Manager** (`platform/tasks/task_manager.py`):
```
...
- Derivation lives in `platform/tasks/stakeholder.py`; manager identity, partner keywords, and org tokens are config-driven (fill them in at setup). Re-runs on every urgency recompute, so tuning takes effect immediately.
...
**Brief System** (`platform/brief/brief.py`):
```
...
**Workflow routes (automatic):** The `UserPromptSubmit` hook matches every message against the `workflow_routes` table and surfaces required platform/skills/pre-checks as "Required workflow:" lines. Follow these before starting work. The table ships EMPTY: add a route whenever a workflow gets skipped or a new skill pattern emerges. Manage routes: `query.py preflight --all` to list, `query.py preflight "task"` to test matching.
...
- **[Manager name]**: [title]. The operator's manager (drives Tier 0/1 urgency; mirror this in `platform/tasks/stakeholder.py` config).
- **[Coworker name]**: [title].

## Safety Rules (also enforce via settings.json deny rules and `platform/hooks/safety_guard.py`)
- External DBs/CMSs (e.g. Notion): list your WRITABLE databases and your PROTECTED (never-write) databases in `platform/hooks/safety_guard.py`. Ships empty; anything not allowlisted is protected by default.
...
|---|---|---|
| `ops_find` | platform/search/rrf_search.py (+ inline FTS fallback) | Fast cross-table search (people, learnings, action items, emails, docs, chat) |
| `ops_cross` | query.py cross | Unified "what do we know about X" ranked view |
| `ops_deep` | platform/search/person_dossier.py + graph | Full person dossier, graph walk, or bio mode (EAV-first current bio + provenance) |
| `ops_query` | platform/tools/query.py | Raw SQL when the MCP shape doesn't fit (e.g. window functions, custom joins) |
| `ops_email_search` | ops.db + gmail-to-sqlite | FTS over both email DBs |
| `ops_tasks` | platform/tasks/task_manager.py | focus / stale / overdue / snooze / resolve / etc |
| `ops_inbox` | platform/comms/inbox_triage.py | status / breaches / daily / classify / classify_new / classify_person / reconcile / scan-asks |
| `ops_brief_ops` | platform/brief/brief.py | status / gather / report / drift_check / registry / new_brief (classify/apply still CLI: needs stdin JSON) |
| `ops_health` | query.py health / drift_check / daily_digest | summary / drift / digest |
| `ops_sync` | platform/brief/daily_sync.py | Pull fresh data from any source list |
| `ops_fabric` | platform/core/mcp_server.py (direct SQL) | Fabric internals: attributes EAV history, relation vocab, batched read-only `probes` (use for multi-check health passes instead of N ops_query calls) |
| `ops_faq` | platform/comms/faq_gate.py + faq_lookup.py | Canonical FAQ search, retrieval-gate tier (draft/draft_cite/escalate) for inbound questions |
| `ops_write` | platform/core/mcp_server.py + platform/comms/steward_bus.py | store a learning / checkpoint / resume / status, plus confirm-gated canonical writes (bus_observation / bus_people). Writes: confirm with the operator before invoking |
...
- `python platform/autonomy/daily_digest.py --print`: yesterday's INGESTED/HEALTH/DRIFT/TRIAGE snapshot. First session of the day shows this automatically.
- `python platform/autonomy/restore_drill.py`: prove backups actually restore. Run monthly.
