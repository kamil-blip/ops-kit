...
def test_tracker_status_and_funnel_json(tmp_path):
    ...
    env = {"SOURCING_DB": str(tmp_path / "sourcing.db")}
    assert run([ROOT / "pipeline" / "demo.py"], env).returncode == 0
    st = run([ROOT / "pipeline" / "tracker.py", "status"], env)
    assert st.returncode == 0 and "conv" in st.stdout
    fj = run([ROOT / "pipeline" / "funnel.py", "--json"], env)
...
def test_templates_lint_rejects_banned_phrase(tmp_path):
    ...
    r = run([ROOT / "pipeline" / "templates.py", "lint", bad])
...
def test_score_gates_then_points():
    ...
    r = run([ROOT / "screening" / "score.py", "--rubric", ROOT / "screening" / "rubrics" / "example-ops-generalist-role.json",
             "--records", ROOT / "screening" / "examples" / "candidates.json"])
...
def test_assign_demo_has_no_conflicts_or_gaps():
    ...
    r = run([ROOT / "screening" / "assign.py", "--demo"])
    assert r.returncode == 0, r.stderr
...
def test_validate_synthetic_reports_spearman():
    ...
    r = run([ROOT / "screening" / "validate.py", "--synthetic"])
    assert r.returncode == 0, r.stderr
...
def test_bias_demo_labels_reviewers():
    ...
    r = run([ROOT / "screening" / "bias.py", "--demo"])
    assert r.returncode == 0, r.stderr
