...
def test_selfcheck_passes_nine_of_nine(py, env, repo_root):
    """tools/selfcheck.py exits 0 and reports every check passed."""
    r = subprocess.run([py, str(repo_root / "tools" / "selfcheck.py")],
...
def test_init_db_is_idempotent(py, env, repo_root):
    ...
    r = subprocess.run([py, str(repo_root / "db" / "init_db.py")],
