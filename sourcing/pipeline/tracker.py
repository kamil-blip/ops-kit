...
Usage:
    python sourcing/pipeline/tracker.py status [SEARCH]      funnel per search: worked, contacted,
                                                    confirmed, delivered, declined, rates
    python sourcing/pipeline/tracker.py chase [SEARCH]       follow-up list in priority tiers
    python sourcing/pipeline/tracker.py stale [--days 7]     non-terminal rows untouched for N days
    python sourcing/pipeline/tracker.py reconcile [SEARCH]   cross-checks between the tables
