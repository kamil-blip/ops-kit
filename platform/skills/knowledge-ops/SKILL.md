---
name: knowledge-ops
description: Gatekeeper for all edits to memory topic files (platform/memory/), skill files, MEMORY.md, and the runbook (template_tasks table). Invoke BEFORE writing to any persistent file (discipline routing: no auto-hook backs this; the model must remember to call it). Ensures information lands in the correct location, prevents duplication, enforces structural rules, and maintains cross-references.
...
**Use this skill when:** Creating/editing memory topic files in `platform/memory/`, skill files in `platform/skills/`, rules/policy files, or the runbook (`template_tasks` table). Restructuring persistent data. Running monthly consolidation. Checking where information should go.
...
- Create or edit a topic file in `platform/memory/`
- Create or edit a file in `platform/skills/`
- Edit `platform/memory/MEMORY.md` (the index)
- Edit the runbook: `python platform/tools/query.py "SELECT * FROM template_tasks ORDER BY phase, step_number"`
...
| Claude-specific workflow, platform config, or decision tree | **Skill** (`platform/skills/[name]/SKILL.md`) | Platform IDs, auto-trigger rules, API quirks |
| Cross-session knowledge about people, processes, tools, or preferences | **Memory** (topic file in `platform/memory/`) | A recurring workflow, CLI gotchas, the operator's feedback |
...
### Skills (`platform/skills/[name]/SKILL.md`)
...
### Memory (`platform/memory/` topic files)

- One topic file per subject, named by type: `feedback_*.md`, `project_*.md`, `reference_*.md`
- One source of truth per topic. Search existing files before creating: `grep -ril "keyword" platform/memory/`
- After creating a new file: add one index line to `platform/memory/MEMORY.md`
...
1. **Does an entry already cover this topic?** Search `platform/memory/`, `platform/skills/`, and the rules files first. Update the existing entry rather than creating a new one.
...
| Skill files | 500 | 20K | Move reference material to `platform/skills/[name]/references/` |
| Rules files | 200 | 10K | Keep focused. Split if covering multiple policies |

**Quick check command:**
```bash
wc -l platform/skills/*/SKILL.md platform/memory/*.md
```
...
- Graph entities: INSERT INTO `entities` via platform/tools/query.py (set `status='reference'` for bulk-imported, un-contacted entities)
...
- Graph: `python platform/tools/query.py "UPDATE entities SET status='archived', updated_at=datetime('now') WHERE id='<entity-id>'"` (relationships preserved)
- Memory files: move to `platform/memory/archive/` and remove from the MEMORY.md index; DB docs: `reference_docs` with `category='archived'`
...
|----------|---------|-----------|-------------------|
| Event-specific | files tied to one event or project cycle | Until wrap-up complete | Move to `platform/memory/archive/` or reference_docs category='archived' |
...
- Move memory files to `platform/memory/archive/`; DB docs to `reference_docs` with `category='archived'`
