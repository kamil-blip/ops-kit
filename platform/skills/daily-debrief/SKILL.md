...
**Command shorthand:** `brief.py` = `python platform/brief/brief.py`, `query.py` = `python platform/tools/query.py`, `task_manager.py` = `python platform/tasks/task_manager.py`. Run from the repo root.

## Core Tool: brief.py

All sync and gathering runs through the brief orchestrator:
```
python platform/brief/brief.py <command>
```
...
|--------|----------|-----------|
| Gmail | platform/brief/daily_sync.py | emails table |
| Discord (registered channels) | platform/brief/daily_sync.py | discord_messages table |
| Signal (registry chats via Beeper; see `brief.py registry`) | brief.py sync --beeper (REST API) + platform/brief/sync_beeper_local.py (full local DB) | beeper_messages table |
| Slack (registry chats via Beeper; see `brief.py registry`) | brief.py sync --beeper (REST API) + platform/brief/sync_beeper_local.py (full local DB) | beeper_messages table |
| Email threads | platform/comms/email_tracker.py sync | email_threads table |
| Meeting transcripts (Granola, optional) | platform/brief/granola_sync.py, or Granola MCP manually in session | reference_docs table |
