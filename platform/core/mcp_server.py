...
# Code lives under <root>/platform/; fall back to the root for older layouts.
_SCRIPT_ROOT = _REPO_ROOT / "platform" if (_REPO_ROOT / "platform").is_dir() else _REPO_ROOT
...
def _find_script(script_name: str):
    ...
    for d in _SCRIPT_DIRS:
        cand = _SCRIPT_ROOT / d / script_name
        if cand.is_file():
            return cand
    return None
...
def _run_script(script_name, *args):
    ...
    if script is None:
        return f"ERROR: script '{script_name}' not found under {_SCRIPT_ROOT}"
