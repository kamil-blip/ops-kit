...
_ROOT_MARKERS = ("config.example.toml", "platform")
...
def _resolve_root() -> Path:
    ...
    for parent in here.parents:
        if (parent / "config.example.toml").is_file() or (parent / "platform" / "db").is_dir():
            return parent
    # Last resort: assume this file lives two levels below the repo root
    # (<root>/platform/core/paths.py).
