...
Most confirmed judges in the source system carry a `contacted_by` and a source; 141 of the 256 confirmed for 2026 do, and the 115 without one are a known gap, not a claim. Of those attributed: 128 were contacted by me directly; 22 came through a referral broker; 17 were captured automatically from inbound mail by the system; 6 came from local hub organisers; 5 from a partner programme's recommendations; 11 from the rest of the team. Acceptance differs by channel: 81% for referrals, 68% for inbound, 39% for my own cold outreach. Signups are attributed by a fixed UTM scheme across 531 published posts, so the channel that brought a participant is known. `sourcing/pipeline/funnel.py` prints the confirmed-by-source mix for every search.
